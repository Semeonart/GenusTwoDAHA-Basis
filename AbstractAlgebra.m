...
FromRules[rules_]:=Plus@@Map[#[[2]]#[[1]]&,rules,1];
...
DefineCompositionAction[algebra_]:=(
	...
	,
		If[!ValueQ[algebra["Id"]],algebra["Id"]=Global`Id]
	];
	algebra["Id"][x_]:=x;
	With[
...
DefineHomomorphismI[\[Psi]_,algebra1_,algebra2_]:=(
	If[!SubsetQ[algebra1["generators"],algebra2["generators"]],
		Print["Failed to define HomomorphismI from ",algebra1," to ",algebra2,", generators are not a subset."];
		Return[Indeterminate];
	];
	DefineHomomorphism[\[Psi],algebra1,algebra2];
	\[Psi][x_?(algebra1["GeneratorQ"])]:=x;
);
...
DefineAutomorphismGroupAlgebra[algebra_,aut_]:=(
	...
	If[$VersionNumber>=12.2,
		If[!ValueQ[aut["Id"],Method->"TrialEvaluation"],aut["Id"]=Global`Id]
	,
		If[!ValueQ[aut["Id"]],aut["Id"]=Global`Id]
	];
...
(*This will allow to keep track of relators used in simplification of the expression*)
DefineFormalQuotient[quot_]:=With[
	{MultF=quot["parentalg"]["NonCommutativeMultiply"],QuotMultF=quot["NonCommutativeMultiply"]}
,
	QuotMultF[X___,MultF[Y_,Z__],W___]:=MultF[X,Y,Z,W];(*Everything that contains MultF is lifted to the top algebra*)
	quot["RelatorQ"][expr_]:=Or@@Map[MatchQ[expr,#]&,quot["relatorpatterns"]];
	QuotMultF[X___,Y_?(quot["RelatorQ"]),Z___]:=MultF[X,Y,Z];(*Everything that contains relators is lifted to the top algebra*)
];
...
,
	...
	If[$VersionNumber>=12.2,
		If[!ValueQ[algebra["GradingMultF"],Method->"TrialEvaluation"],algebra["GradingMultF"]=Plus];
		If[!ValueQ[algebra["GradedOperationQ"],Method->"TrialEvaluation"],SetMultiplicationZeroDegree[]]
	,
		If[!ValueQ[algebra["GradingMultF"]],algebra["GradingMultF"]=Plus];
		If[!ValueQ[algebra["GradedOperationQ"]],SetMultiplicationZeroDegree[]]
	];
	...
	If[algebra["Deg"][Times]==0,
		algebra["Deg"][Power[x_,n_]]:=n algebra["Deg"][x]
	];
...
,
	...
	,
		...
		If[DirectoryQ[ideal["cachedir"]],Export[filename,ans]];
		Return[ans];
	];
